...
$(document).ready(function () {
    ...
    $('.button_hide').click(function(event){
        ...
        $('.block_text').attr("style", "display:none");
    })

    $(window).scroll(function(event) {
        let st = $(this).scrollTop();
        if(st > 15){
            $('.header_conteiner,.header_logo').addClass('active');
           
        }else{
            $('.header_conteiner,.header_logo').removeClass("active");
...
const orderModal = document.querySelector('.modal__btn');
...
//шаблон товара
const renderItem = (text,img,price,id,color) => {
    return `
    <li class="bag_tovar_item" data-nameItem="${text}" data-id="${id}">
    <div class="tovar_img">
    ...
    <div class="tovar_bottom">
        <span class="tovar_title">${text}</span>
        <div class="size" >
            <span class="size_it" data-size="XS"  >XS</span>
            <span class="size_it" data-size="S" >S</span>
            <span class="size_it"  data-size="M" >M</span>
            <span class="size_it" data-size="L" >L</span>
            <span class="size_it" data-size="XL" >XL</span>
            <span class="size_it" data-size="2XL" >2XL</span>
            <span class="size_it" data-size="3XL">3XL</span>
            <span class="size_it" data-size="4XL" >4XL</span>
            <span class="size_it"  data-size="5XL" >5XL</span>
        </div>
        <span class="tovar__color">Color:<span class="color__item" data-coloritem="${color}">${color}</span></span>
...
/*
const renderItemModal = (text,img,price,id,quantityProdut) => {
    return `
    <li class="order-modal__item" data-id="${id}">
		<article class="order-modal__product">
			<img src="${img}" alt="">
			<div class="order-modal__text">
				<h3 class="order-modal__title">${text}</h3>
                <span class="order-modal__price">${price}</span>
               
                <span class="order-modal__quantity">${quantityProdut}</span>
			</div>
			<button class="order-modal__delete">Delete</button>
		</article>
	</li>
    `;
};*/
...
/*
const order = () => {
    let array = document.querySelector('.bag_list').children;
            let fullPriceOrder = document.querySelector('.full_price').textContent;
            
    
            
            
            
            console.log(document.querySelector('.bag_list').children);
            for(item of array) {
               
               
                let price = item.querySelector('.products_price').textContent;
                let text = item.querySelector('.tovar_title').textContent;
                let img = item.querySelector('.tovar_photo img').getAttribute('src');
                let quantityProdut = item.querySelector('.input_item').textContent;
                

                let id = item.dataset.id;
                
                document.querySelector('.order_list_page').insertAdjacentHTML('afterbegin', renderItemModal(text,img,price,id,quantityProdut));
...
catalogItem.forEach((el) => {   
    el.querySelector('.add').addEventListener('click',(e)=>{
        ...
        let color = el.querySelector('.active__color').dataset.color;
        console.log(color);
        
        plusFullPrice(price);
        printFullPrice();
        document.querySelector('.bag_list').insertAdjacentHTML('afterbegin', renderItem(text,img,price,id,color));
...
const sizeItem = () => {
    let size = document.querySelector('.size_colums').querySelector('.active_btn').dataset.size;

    itemsBasket.querySelector(`.size_it[data-size="${size}"]`).classList.add('active_size');
    updateStorages();
    console.log(size);
}
...
const modal = new GraphModal({
    isOpen:(modal) => {
        ...
        let array = document.querySelector('.bag_list').children;
            ...
            document.querySelector('.order-modal__summ span').textContent = `${fullPriceOrder}`;
            console.log(document.querySelector('.bag_list').children);
            for(item of array) {
                ...
                let img = item.querySelector('.tovar_photo img').getAttribute('src');
                let quantityProdut = item.querySelector('.input_item').textContent;
                let sizeItem = item.querySelector('.active_size').textContent;

                let id = item.dataset.id;
                
                if(document.querySelector('.bag_list').children){
                    orderModalList.insertAdjacentHTML('afterbegin', renderItemModal(text,img,price,id,quantityProdut,sizeItem));
                }else{
                    return 0;
                }
...
document.querySelector('.order').addEventListener('submit',(e)=>{
    ...
    let email = self.querySelector('[name="phone"]').value;
...
const sliders = document.querySelector('.catalog_slider');
...
let mySwipers = new Swiper(sliders ,{
    slidesPerView:1,
...
if(color){
    color.addEventListener('click', (e) =>{
        if(e.target.classList.contains('color_btn')){
            document.querySelectorAll('.color_btn').forEach((el) => el.classList.remove('active__color'));
...
const size2 = document.querySelector('.bag_list');
...
if(size2){
    ...
    size2.addEventListener('click', (e)=>{
        ...
        if(e.target.classList.contains("size_it")){
            e.target.closest('.bag_tovar_item').querySelectorAll('.size_it').forEach((el) => el.classList.remove('active_size'));
            

            e.target.classList.add('active_size');
            updateStorages();
        }
    })
}
...
document.querySelector('.number_colums1').addEventListener('click', (e) =>{
    let colums = e.target.dataset.colums;
    document.querySelector('.catalog_tovar_colums').dataset.grid = colums;
...
const createChoiceItem = (text) => {
    return (
        `<button class="filter_item" data-choice-text="${text}">
            ${text}
...
filter.forEach(el => {
    el.querySelector('input').addEventListener('change', (e) => {
        ...
        let activeCheckbox = document.querySelector('.custom--active');

        if(activeCheckbox){
            document.querySelector('.catalog_tovar_center').style.display = 'block';
        }else{
            document.querySelector('.catalog_tovar_center').style.display = 'none';
        }
    })
})
...
document.querySelector('.filter_catalog').addEventListener('click', (el) => {
    if(el.target.classList.contains('filter_item')){
        el.target.remove();
        
        let text = el.target.textContent.trimLeft().trimRight();

        document.querySelector(`[data-text="${text}"]`).querySelector('input').checked = false;
        document.querySelector(`[data-text="${text}"]`).classList.remove('custom--active');
        
    }
    if(el.target.classList.contains('clear_button')){
        Array.from(el.currentTarget.children).forEach(el => {
            if(!el.classList.contains('clear_button')){
                el.remove();
                filter.forEach(el =>{
    ...
    if(el.currentTarget.children.length === 1 ){
        document.querySelector('.catalog_tovar_center').style.display = 'none';
    }
...
/*window.addEventListener('scroll',function(){
	...
	header.classList.toggle('slick', window.scrollY > 0);
});*/
...
if(document.querySelector('#select')){
    const sortList = document.querySelector('#select');
sortList.addEventListener('change', function(){
    if(sortList.options[sortList.selectedIndex].value == 'value1'){
        return mySort();
    }
    if(sortList.options[sortList.selectedIndex].value == 'value2'){
        return mySortDesc();
    }
})
...
function mySort() {
    let nav = document.querySelector('.catalog_tovar_colums');
    for (let i = 0; i < nav.children.length; i++) {
        for (let j = i; j < nav.children.length; j++) {
            if(+nav.children[i].dataset.price > +nav.children[j].dataset.price){
...
const ROOT_PRODUCTS = document.getElementById('bag');;
